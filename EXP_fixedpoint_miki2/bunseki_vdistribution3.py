...
def plot_values5(d, q, dist, n, def_ang, f_dir, fname, dd, qid):
	...
	fig = plt.figure(figsize=(100,10))	#figsize=（横幅、縦幅）
	#queryimgのピクセル数
	...
	#類似度グラフ
	ax11 = fig.add_subplot(3,3,2)#グラフを描画する場所を定義
	si=(Bc+C)/(A+B+C+D+E+0.0001) #類似度の評価方法を定義
	p=si.argmax()
	...
	#存在確率分布を作成
	maxd = dd.argmax()
	sonzai = fig.add_subplot(3,3,3)#グラフを描画する場所を定義
	sonzai.plot([Pbst,Pbst], [dd, dd], linestyle='solid',lw=0.1,  color ='grey', label="normal distribution")#plt.legend(loc="lower right")
