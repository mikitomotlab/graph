...
def get_GT_position(f_dir):
	...
	x, y, z, th, ph =0.3, 0.7, 1.2, -47.2, 0.0 #query画像ごとに真値座標は変化する必要がある
...
def calc_distance(q, db):
	...
	for i, dummy in enumerate(db[:,0]):
		...
		d = np.array([q[1],q[2],q[3],q[4]*np.pi/180,q[5]*np.pi/180])
		norm[i] = np.linalg.norm( c - d )
		
		e = np.array((db[i,4]*np.pi/180, db[i,5]*np.pi/180))
		f = np.array([q[4]*np.pi/180,q[5]*np.pi/180])
...
def main():
	files_w_n = inputfile_listing()#csvファイルを選択
	end=0
	n=0
	while(end >= 0):
		...
		fname = fullname[p+25:-4]		#fname 絶対パスが変わると文字数が変わり、変更が必要
