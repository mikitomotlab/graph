...
def inputfile_listing():
	...
	number=np.arange(len(results))
	results_with_no = np.hstack([np.vstack(number), np.vstack(results)])
	
	return files_with_no, results_with_no
...
def main():
		files_w_n, results_w_n = inputfile_listing()#csvファイルを選択
		end=0
		n=0
		qnum = 1
	...
	#選択したresultのcsvファイル
		...
		print(resultsname)
		query_number = len(results_w_n)
	#UMapマッチング結果のcsvファイルを選択（queryがごとのためquery枚数分繰り返す）
		while(qnum == query_number):
			...
			#n=int(input())
					...
					#選択したqueryの結果csvファイル	
						...
						p = fullname.find('EXP_fixedpoint_try')
						fname = fullname[p+25:-4]	
	...
	#実験結果をres_dataへ格納する
		...
		#結果の描画
						plot_values3(data, query, dist, norm, def_ang, f_dir, fname)
		
						qnum = qnum + 1
	

def main_d():
	fullname = "C:/Users/tmem/Desktop/DEMO_&_EXP/EXP_fixedpoint/query1001/type1_PB2_o_320_180_z_const_ver1_GD_10_0.csv"
	
	p=fullname.find('EXP_fixedpoint')
	fname = fullname[p+25:]		#fname 絶対パスが変わると文字数が変わり、変更が必要
	f_dir = fullname[p+15:p+24]	#p_directory

	data = read_txt(fullname)
	query = get_query_info(fullname)
	
	dist, norm, def_ang = calc_distance(query,data)
	
	np.savetxt('./dist.csv',dist,delimiter=',')
	np.savetxt('./norm.csv',norm,delimiter=',')
	np.savetxt('./def_ang.csv',def_ang,delimiter=',')
	
	#d_best_th = best_th(data)
	print("(" + f_dir + ")" + fname)
	plot_values(data, query, dist, norm, def_ang, f_dir, fname)
